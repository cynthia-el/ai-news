...
1. 标题含"避坑"、"翻车"等纯C端消费体验词（注意：企业运营方法论/新技术应用教程类除外）
...
5. 纯品牌软文/产品种草/招商加盟（无实质信息，如"选XX就对了"、"限时优惠"、"诚招代理"）
6. B2B采购询价（"需要采购XX"、"求购XX材料"、"咨询XX价格"）
7. 内容乱码、无法阅读、全是符号

【重要：以下类型 NOT 排除 — 应正常评分】
- 企业新技术/新工艺/新材料发布（如"无醛添加技术突破"、"量子净界板材商用"）
- 企业专利授权/技术转化（如"获国家发明专利"、"技术成果通过鉴定"）
- 企业新运营模式/新渠道策略（如"整木定制数字化升级"、"C2M模式落地"）
- 企业智能制造/产线升级（如"智能工厂投产"、"工业机器人应用"）
- 企业ESG/绿色生产实践（如"碳中和生产线"、"水性漆替代"）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【五维评分标准 — 满分10分，≥6分入选】

1. 信源权威性（0-2分）
   - 2分：国家部委/证劵研报/头部企业官方/行业协会/专利局
   - 1分：行业垂直媒体/知名财经媒体/企业官方发布
   - 0分：自媒体/未知来源/营销号

2. 数据密度（0-2分）
   - 2分：含具体数字（金额、百分比、产能、规模、政策文号、专利号、技术参数）
   - 1分：有定性描述但缺乏具体数据
   - 0分：无实质内容，纯观点/口号

3. 战略相关性（0-3分）
   - 3分：直接影响竞争格局/政策合规/投资决策/技术路线/供应链结构
   - 2分：行业趋势参考，或企业级创新/专利/新运营模式有参考价值
   - 1分：一般资讯，战略价值有限
   - 0分：与战略决策完全无关
   【核心赛道加分】若资讯直接涉及人造板产业链（产能、环保标准、胶粘剂、饰面工艺、出口等）或全屋定制/定制家居（渠道、整装、财报、门店、并购、柔性生产等），本维度可额外上浮0.5-1分
   【企业创新加分】若资讯涉及企业新技术突破、专利授权、智能制造升级、新运营模式（如C2M、数字化定制），本维度可上浮0.5-1分
...
5. 独家性/实操价值（0-2分）
   - 2分：独家政策解读/首发数据/重大战略宣布/企业首发的技术突破
   - 1.5分：企业具体的专利信息、产线升级细节、运营数据公开
   - 1分：常规报道
   - 0分：转载/群发通稿
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【行业聚焦方向 — 企业级创新优先，核心赛道加码】
核心关注以下产业领域，特别重视企业层面的具体创新动作与竞争/产能信号：

一、新产能与产能动态（最高优先级）
- 人造板企业：新产线投产、扩产计划、产能利用率、区域产能布局、进口/国产设备、压机升级
- 定制家居企业：智能工厂投产、柔性生产线扩建、产能爬坡、交付能力提升
- 产业链上下游：木材、胶粘剂、五金配件供给变化与价格走势

二、竞品动态与渠道变化（最高优先级）
- 头部企业财报：营收、净利润、毛利率、分渠道收入
- 门店与渠道：开店/关店、经销商变化、整装合作、设计师渠道、电商渠道
- 战略动作：并购、收购、合资、出海、品牌升级、品类延伸
- 整装/高定：整装模式进展、高定品牌布局、整木定制动态

三、技术创新与突破（高优先级）
- 人造板企业：无醛添加技术、新型胶黏剂（大豆胶、MDI胶）、饰面工艺突破、压贴技术升级
- 定制家居企业：数字化设计系统、柔性生产线、C2M定制模式、智能排产系统
- 智能制造：工业机器人应用、AI质检、MES系统部署、智能仓储物流
...
五、新运营方式与模式创新（中高优先级）
- 整木定制/全屋定制企业的运营效率提升案例
- 新材料/新工艺在企业的首次商用
- 企业ESG实践（绿色工厂、零碳产线、废水废气治理）
- 供应链创新（集中采购、协同设计、数字化供应链）
  ...
  try {
    ...
    return results.map((r, idx) => {
      ...
      if (!hasSecondary) {
        ...
        // 未直接命中时，根据细分领域关键词推断
        if (!matched) {
          if (/人造板|刨花板|胶合板|纤维板|OSB|饰面板|板材|ENF|甲醛|无醛|胶粘剂|林业|木材/.test(text)) {
            tags.unshift('人造板')
          } else if (/全屋定制|定制家居|整木定制|整装|高定|橱柜|衣柜|木门|定制家具|柔性生产|C2M/.test(text)) {
            tags.unshift('全屋定制')
          } else if (/木结构|装配式/.test(text)) {
            tags.unshift('木结构')
          } else if (/智能照明|智能安防|IoT|智能家居|AI.{0,3}家居/.test(text)) {
            tags.unshift('智能家居')
          } else if (/绿色建材|双碳|负碳|以旧换新|绿色.{0,2}建筑/.test(text)) {
            tags.unshift('绿色建材')
          } else if (/陶瓷|瓷砖|岩板|卫浴|洁具/.test(text)) {
            tags.unshift('陶瓷卫浴')
          } else if (/门窗|厨卫|系统门窗|铝门窗/.test(text)) {
            tags.unshift('门窗厨卫')
          } else if (/家具|软体|办公|红木|床垫|沙发/.test(text)) {
            tags.unshift('家具制造')
          }
          // 仍无法推断则留空，不默认全屋定制
        }
